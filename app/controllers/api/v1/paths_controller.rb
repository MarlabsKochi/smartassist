class Api::V1::PathsController < Api::V1::BaseController
  ...
  def dummys
    ...
    unless start_point          
      ...
      real_one = path[0]
      real_two = path[1]
      array_for_real << real_one
      array_for_real << real_two
      first_point = co_ordinates[path[0]]
      second_point = co_ordinates[path[1]]
      one_one= first_point.first.first
      one_two= first_point.first.last
  ...
  to_return = []
        array_for_real.each do |point|
         ob = Dijkstra.new(point,end_point, paths_stored)
         each_array_object = []
         paths = []
           ob.shortest_path.each {|a| paths<< copy[a]}
           p "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$"
           p paths
          each_array_object << "cost" => ob.cost
          each_array_object << "path" => paths
         response ={
           :cost => ob.cost,
             :path => paths
         }
